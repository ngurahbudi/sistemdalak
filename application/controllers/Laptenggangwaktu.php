<?
	class Laptenggangwaktu extends CI_Controller
	{
		...
		public function __construct()
		{
        		...
        		$this->load->model('M_laptenggangwaktu');
		...
		{	// accpet hanya ketika session terbentuk
			if(isset($this->session->username))
			{
				...
				//$data['custom_column']= $this->M_laptenggangwaktu->SelectCustomColumn(); 
				//$data['alldata']= $this->M_laptenggangwaktu->SelectAllData();
				$this->load->view('Laptenggangwaktu/v_laptenggangwaktu',$data);  
		...
		public function search() 
		{
			...
			//$data['custom_column']= $this->M_laptenggangwaktu->SelectCustomColumn();
 			$where="";
			$status_filter="";
			$where .=" and i.targetwaktu >= '". $this->mycombo->FormatYMD($this->input->post('txttglturun')) ."' and i.targetwaktu <= '" . $this->mycombo->FormatYMD($this->input->post('txttglturun2')) . "'";
			...
			{
				//$data['alldata']= $this->M_laptenggangwaktu->SelectAllData($where);
				 
				$this->M_laptenggangwaktu->export($this->M_laptenggangwaktu->SelectAllData($where),$status_filter);
				$this->load->view('Laptenggangwaktu/v_laptenggangwaktu',$data);  	
			} 
			if($this->input->post('cmdFilter2') )
			{ 
				$this->M_laptenggangwaktu->export_custom($where,$status_filter);
			 	$this->load->view('Laptenggangwaktu/v_laptenggangwaktu',$data);  
			}
    	}
