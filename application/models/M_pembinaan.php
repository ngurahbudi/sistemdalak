	...
	class M_pembinaan extends CI_model
	{
		...
		public function SelectAllData($where="") 
		{
			$sql = "SELECT a.jadwal_id,   a.jadwal_tgl, c.prs_id, c.prs_nama, c.prs_lokasidet, d.jalan_id, d.namajalan, f.kelurahan,
					...
					WHERE h.jenismonitoring_id='PB' " . $where  ;  
		...
		function Simpan() 
		{
        	$idnew=$this->M_pembinaan->CreateNewID('PB',4);  
  			$bantu=$this->input->post('txttarget');
			($bantu =="" || $bantu=="00-00-0000" ) ? $tgltarget=null : $tgltarget=$this->mycombo->FormatYMD($bantu);
			$data = array(
					'monit_id' 			=> $idnew,
					'jadwal_id' 		=> $this->input->post('txtID'),
					'catatanlapangan' 	=> $this->input->post('txtcatatan'),
					'targetwaktu' 		=> $tgltarget,
			...
			//upload file
			$uploadsukses=$this->M_pembinaan->upload_file($this->input->post('txtperusahaan'),$idnew,"PB");
			//=====
    	...
    	function Update() 
		{
			...
			$this->db->update("tb_perusahaan",$data2); // update di tabel perusahaan set klas masalah dan kategory perusahaan
			//upload file
			$uploadsukses=$this->M_pembinaan->upload_file($this->input->post('txtperusahaan'),$this->input->post('txtIDmonit'),"PB");
			//=====
			if ($this->db->trans_status() === FALSE)
			{
				$this->db->trans_rollback();
		...
		{
			$sudah=$this->M_pembinaan->CekSudahSurvey($prs_id,$id);
			$data= array(
					'sudah_PB'			=> $sudah 
			);
		...
		function CreateNewID($prefix,$digit)
		{
			...
			$sql.=" WHERE monit_id <>'' and   substr(monit_id,1,2) ='PB' ";
						...
						{ 
							...
							{	
							  // proses upload ke server
								$det=$this->M_pembinaan->getNewUrutan("id_file","tb_file_fotomonitoring", " and monit_id='$monitid'");
