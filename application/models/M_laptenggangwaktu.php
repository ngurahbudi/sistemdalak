<?
	class M_laptenggangwaktu extends CI_model
	{
		public $namatabel;
		...
		public function SelectAllData($where="") 
		{
			...
			WHERE a.prs_id<>'0' and h.jenismonitoring_id='PB'  " . $where ;  
		...
		public function export($data,$status_filter="" ) 
		{
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('A1' , "LAPORAN TENGGANG WAKTU");
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, "Tgl.Tempo");
			...
			$objPHPExcel->getActiveSheet()->getStyle('L'.$rowHeader )->applyFromArray($styleThinBlackBorderOutline);
			$i=0;
			$rowCount++;
			// fill data
			foreach($data as $value)
			{
				...
				$objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, $this->mycombo->FormatYMD($value['targetwaktu']));
				$rowCount++; 
			} 
	
			// Set thin black border outline around column
			$borderindex = sprintf('A'.$rowHeader.':L%d',$rowCount-1  );
			...
			$objWriter->save('./lapexcell/Lap-tenggangwaktu.xlsx'); 
	
			$this->load->helper('download');
			force_download('./lapexcell/Lap-tenggangwaktu.xlsx', NULL);
		}
