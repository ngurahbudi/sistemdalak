<?
	class M_lapjadwal extends CI_model
	{
		public $namatabel;
		...
		public function SelectAllData($where="") 
		{
			...
			INNER JOIN tb_jenis_monitoring j on h.jenismonitoring_id=j.jenismonitoring_id
		...
		public function export($data,$status_filter="" ) 
		{
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('A1' , "JADWAL PELAKSANAAN KEGIATAN BIDANG DALAK");
			...
			$rowHeader=$rowCount;
			$objPHPExcel->getActiveSheet()->getStyle('A'.$rowHeader.':L'.$rowHeader)->getFont()->setBold(true); 
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, "Bidang Usaha");
			$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, "Monitoring");
			...
			foreach($data as $value)
			{
				...
				$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $this->mycombo->NamaHari($value['jadwal_tgl']) . ", " .$this->mycombo->FormatYMD($value['jadwal_tgl'])); 
				...
				$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, $value['prs_bidangusaha']); 
				$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, $value['jenismonitoring']); 
 
				$rowCount++; 
			} 
	
			// Set thin black border outline around column
			$borderindex = sprintf('A'.$rowHeader.':H%d',$rowCount-1  );
			...
			$objWriter->save('./lapexcell/Lap-jadwal.xlsx'); 
	
			$this->load->helper('download');
			force_download('./lapexcell/Lap-jadwal.xlsx', NULL);
		}
