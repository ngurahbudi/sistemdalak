	...
	class M_pengawasan extends CI_model
	{
		...
		public function SelectAllData($where="") 
		{
			$sql = "SELECT a.jadwal_id,   a.jadwal_tgl, c.prs_id, c.prs_nama, c.prs_lokasidet, d.jalan_id, d.namajalan, f.kelurahan,
					...
					WHERE h.jenismonitoring_id='PS' " . $where  ;  
		...
		function Simpan() 
		{
        	$idnew=$this->M_pengawasan->CreateNewID('PS',4);  
			...
			//upload file
			$uploadsukses=$this->M_pengawasan->upload_file($this->input->post('txtperusahaan'),$idnew,"PS");
			//=====
    	...
    	function Update() 
		{
			...
			$uploadsukses=$this->M_pengawasan->upload_file($this->input->post('txtperusahaan'),$this->input->post('txtIDmonit'),"PS");
			//=====
			if ($this->db->trans_status() === FALSE)
			{
				$this->db->trans_rollback();
		...
		{
			$sudah=$this->M_pengawasan->CekSudahSurvey($prs_id,$id);
			$data= array(
					'sudah_PS'			=> $sudah 
			);
		...
		function CreateNewID($prefix,$digit)
		{
			...
			$sql.=" WHERE monit_id <>'' and   substr(monit_id,1,2) ='PS' ";
						...
						{ 
							...
							{	
							  // proses upload ke server
								$det=$this->M_pengawasan->getNewUrutan("id_file","tb_file_fotomonitoring", " and monit_id='$monitid'");
