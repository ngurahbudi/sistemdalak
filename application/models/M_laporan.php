<?
	class M_laporan extends CI_model
	{
		...
		public function export($data,$status_filter="") 
		{
			...
			$rowHeader=$rowCount;
			$objPHPExcel->getActiveSheet()->getStyle('A'.$rowHeader.':G'.$rowHeader)->getFont()->setBold(true); 
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, "Bidang Usaha");
			...
			foreach($data as $value)
			{
				...
				$objPHPExcel->getActiveSheet()->setCellValueExplicit('C'.$rowCount, $value['prs_telpkantor'], PHPExcel_Cell_DataType::TYPE_STRING);
				$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $value['prs_bidangusaha']); 
				$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, $value['namajalan'].", ".$value['prs_lokasidet'].", ".$value['kelurahan'].",".$value['kecamatan']); 
			...
			$borderindex = sprintf('A'.$rowHeader.':G%d',$rowCount-1  );
